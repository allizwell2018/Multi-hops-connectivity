...
graph_convolution_data = scipy.io.loadmat('/DATA/data2/dywu/graph_convolution_data_face360.mat')
...
l1_reg = 0  #1e-3 
element_sparse = 0; group_sparse = 1; l2_sparse = 0; sparse_correction = 1
...
print(element_sparse,group_sparse,l2_sparse,sparse_correction,milestones,perm_individual)
...
activation_thresh = 0.9750   #face-shape: 0.6480  0.9750  1.2690 face-avg: 0.7315 0.9557 1.1358
...
class wsgcn_multi_net(torch.nn.Module):
    def __init__(self, multi=1, in_channels=1, out_channels=1):
        ...
        if batchnorm:
            self.normalizations = nn.ModuleList([nn.BatchNorm1d(num_nodes)  for i in range(stepnum)])
        
        if batchnormF:
            self.normalizationsF = nn.BatchNorm1d(num_nodes) 
        # self.convf = wsgcn_multi(in_channels, out_channels, num_nodes, 
        #             multi=multi, multi_aggre='concat', featureTrans=False, nonsharing=True)
    ...
    def forward(self, adj, x, timestep=1):
        if nonsharing:
            ...
            for i in range(timestep-1):          
                ...
                if batchnorm:
                    x = self.normalizations[i](x)  ###效果不明显
                if relu:
                    x = F.relu(x)  ## 是否需要relu，在输入前加入relu吗，用relu模拟神经活动和bold的分离？
                x = F.dropout(x, p = dropout, training=self.training)                  
            x = self.convs[-1](x, adj, self.weight2, self.weight, self.bias, add_loop=add_loop, residue=residue)
            if batchnormF:
                x = self.normalizationsF(x) 
...
def test(layer, data_loader):
    with torch.no_grad():
        ...
        for i, batch in enumerate(data_loader):  # 每一步 loader 释放一小批数据用来学习
            ...
            batch_cdti, batch_task = batch[0].to(device), batch[1].to(device)             
            ...
            for k in range(layer):                  
                batch_task_initials = net[k](batch_cdti,batch_task_initials, timestep=stepnum)
                    
            outputs = net[layer](batch_cdti,batch_task_initials, timestep=stepnum)
...
for rep in range(rep_num):
    ...
    for layer in range(layernum):
        ...
        if not nonsharing:
            grad_sharing = torch.zeros_like(net[layer].weight2)
            grad_sharing_norm1 = 0; grad_sharing_norm2 = 0;  
            grad_sharing_norm0_list = []; grad_sharing_norm1_list = []; grad_sharing_norm2_list = [];  
            weight_sharing_norm_list = [torch.norm(net[layer].weight2).item()]
        ...
        for epoch in range(epochs):   
            for i, batch in enumerate(train_val_loader):  
                ...
                if add_noise:
                    if noise1:
                        var_batch = torch.var(batch_cdti)
                    elif noise2:
                        var_batch = torch.var(batch_cdti,0)
                    batch_cdti += torch.randn_like(batch_cdti)*torch.sqrt(var_batch/add_noise)
                if add_noise_y:
                ...
                if l1_reg:
                    if nonsharing:
                        ...
                        if element_sparse:
                            for step in range(stepnum):
                                l1_regulization += torch.sum(torch.abs(net[layer].convs[step].weight_nonsharing))
                            # l1_regulization = l1_regulization / stepnum
                        if group_sparse:
                            for step in range(stepnum):
                                norm_weight = torch.norm(net[layer].convs[step].weight_nonsharing, dim=1)
                                norm_weight0 = torch.norm(net[layer].convs[step].weight_nonsharing, dim=0)
                                diag_weight = torch.diag(net[layer].convs[step].weight_nonsharing)
                                norm_bias = torch.norm(net[layer].convs[step].bias_nonsharing, dim=1)
                                # l1_regulization += torch.sum(torch.sqrt(norm_weight**2 + norm_bias**2))
                                # l1_regulization += torch.sum(torch.sqrt(norm_weight**2+norm_bias**2+norm_weight0**2))
                                l1_regulization += torch.sum(torch.sqrt(norm_weight**2+norm_bias**2+norm_weight0**2-diag_weight**2))
                                # l1_regulization += torch.sum(torch.sqrt(norm_weight**2+norm_bias**2)+torch.sqrt(norm_weight0**2))
                                # l1_regulization += torch.sum(torch.norm(net[layer].convs[step].weight_nonsharing, dim=1))
                                if l2_sparse:
                                    l1_regulization += torch.sum(norm_weight**2)
                            # sparse_correction=2.5; 
                            l1_regulization = l1_regulization*sparse_correction
                    else:
                        l1_regulization = 0
                        if element_sparse:
                            l1_regulization += torch.sum(torch.abs(net[layer].weight2))
                        if group_sparse:
                            norm_weight = torch.norm(net[layer].weight2, dim=1)
                            norm_bias = torch.norm(net[layer].bias, dim=1)
                            l1_regulization += torch.sum(torch.sqrt(norm_weight**2 + norm_bias**2))
                            # l1_regulization += torch.sum(torch.norm(net[layer].weight2, dim=1))

                    loss = loss + l1_reg*l1_regulization

                loss.backward()
                optimizer.step()  

                if not nonsharing:
                    weight_sharing_norm = torch.norm(net[layer].weight2); 
                    weight_sharing_norm_list.append(weight_sharing_norm.item())
                    grad_sharing += (net[layer].weight2.grad)**2 
                    grad_sharing_norm0 = torch.norm(net[layer].weight2.grad); 
                    grad_sharing_norm1 += torch.norm(net[layer].weight2.grad); 
                    grad_sharing_norm2 = 0.999*grad_sharing_norm2 + 0.001*torch.norm(net[layer].weight2.grad);            
                    grad_sharing_norm0_list.append(grad_sharing_norm0.item())
                    grad_sharing_norm1_list.append(grad_sharing_norm1.item())
                    grad_sharing_norm2_list.append(grad_sharing_norm2.item())                
